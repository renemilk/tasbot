...
def gethand(status):
	...
	bit5 = (status >> 16) & 1
	bit6 = (status >> 17) & 1
	hand = (pow(2, 0) * bit0 + pow(2, 1) * bit1 + pow(2, 2) * bit2 +
			pow(2, 3) * bit3 + pow(2, 4) * bit4 + pow(2, 5) * bit5 +
			pow(2, 6) * bit6 + pow(2, 7) * bit4)
	return hand
